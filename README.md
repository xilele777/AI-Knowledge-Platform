...
**10. 测试与工程化** — [src/utils/__tests__/](src/utils/__tests__/) / [e2e/](e2e/) / [.github/workflows/](.github/workflows/)
Vitest 覆盖 SSE 解析、流式切分、think 状态机、引用解析等核心纯函数；Playwright E2E 全 mock 后端（localStorage 注入登录态 + 拦截 PostgREST/SSE），覆盖「提问 → meta authoritative sources → 流式回答 → 思考面板」关键路径；GitHub Actions 双 workflow：CI（lint + 单测 + 构建 / E2E 双 job）+ push main 自动部署 Cloudflare Pages。

## 快速开始

### 前置条件

- Node.js >= 20.19（推荐 22，与 CI 一致）
...
supabase functions deploy admin-user-role
```
...
| `npm run preview` | 构建后用 `wrangler dev` 本地预览生产产物 |
| `npm test` | Vitest 单元测试（100+ 用例） |
...
线上站点托管在 Cloudflare Pages，静态资源经 CDN 边缘分发：

- **自动部署**：push 到 `main` 触发 [.github/workflows/pages-deployment.yml](.github/workflows/pages-deployment.yml)，CI 构建后通过 `wrangler pages deploy` 发布（Supabase 环境变量与 Cloudflare Token 均配置在 GitHub 仓库的 Variables / Secrets 中）
- **手动部署**：本地 `npm run deploy`（需 `wrangler login`）
- **缓存策略**：[public/_headers](public/_headers) 定义差异化缓存 —— HTML `no-cache`，内容哈希命名的静态资源 `max-age=31536000, immutable`
- **SPA 路由**：[wrangler.jsonc](wrangler.jsonc) 中 `not_found_handling: "single-page-application"`，所有前端路由回退到 `index.html`
...
docs/             亮点深挖与优化方案文档
supabase/
├── sql/          21 个数据库迁移脚本（含 pgvector 双列迁移）
└── functions/    Edge Functions（ai-chat / ai-embeddings / admin-analytics / admin-user-role）
.github/          CI（lint + 单测 + 构建 / E2E 双 job）+ Cloudflare Pages 自动部署
```
...
- **单元测试**：Vitest 覆盖切片、双路检索评分、RRF 融合、SSE 解析、流式切分、think 状态机、引用解析、上下文裁剪等核心纯函数（100+ 用例），`npm test`
- **E2E 测试**：Playwright 全 mock 后端跑通「登录态 → 提问 → 流式回答 → 思考面板」关键路径，无密钥可运行，`npm run test:e2e`
- **CI/CD**：GitHub Actions 双 workflow —— CI（lint + 单测 + 构建 / E2E 双 job，push 与 PR 触发）+ push main 自动构建并部署到 Cloudflare Pages
- **代码规范**：ESLint（flat config，vue + typescript-eslint）+ Prettier，`npm run lint` / `npm run format`

## 文档

- [docs/Cloudflare-Pages部署与CDN缓存说明.md](docs/Cloudflare-Pages部署与CDN缓存说明.md) — 部署链路与 CDN 缓存策略详解
